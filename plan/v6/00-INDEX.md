# missless.co V6 — 최종 구현 계획서 (Phase 분할)
...
> V5 → V6 핵심 변경: 모델명 검증 완료 + 사전준비 문서 분리 + Lyria 모델명 수정 + 추후개발 분리
...
| Image 모델명(flash) | `gemini-2.5-flash-image` (미검증) | **`gemini-2.5-flash-preview-image-generation`** (GA) | 실제 모델명 검증 완료 |
| Image 모델명(pro) | `gemini-3-pro-image-preview` (미검증) | **`imagen-3.0-generate-002`** (Imagen 3) | 실제 모델명 검증 완료 |
| 비디오 분석 모델 | `gemini-3.1-pro-preview` (미검증) | **`gemini-2.5-pro-preview-03-25`** | 실제 모델명 검증 완료 |
...
│  │                                                        │ │
│  │  change_atmosphere → Lyria BGM 생성 or 프리셋          │ │
...
| 이미지 생성 (flash) | `gemini-2.5-flash-preview-image-generation` | 동일 | 1-3초 프리뷰 |
| 이미지 생성 (pro) | `imagen-3.0-generate-002` | 동일 | 고품질 (Imagen 3) |
| 비디오/URL 분석 | `gemini-2.5-pro-preview-03-25` | 동일 | YouTube URL 직접 분석 |
| 페르소나 생성 | `gemini-2.5-pro-preview-03-25` | 동일 | JSON 구조화 출력 |
| BGM 생성 | `models/lyria-realtime-exp` | 동일 | 실험적 모델 |
...
| T15 | BGM 시스템 + change_atmosphere | 중 | 0.5 | Lyria BGM 생성 또는 프리셋 |
...
| rich, mixed-media responses | 음성(HD 프리셋) + 이미지(Progressive) + BGM(Lyria) 실시간 | ✅ |
...
| YouTube URL 분석 | 공개/일부공개 영상만 | 비공개 → 갤러리 Fallback |
| Lyria RealTime | 실험적 모델 (불안정 가능) | 프리셋 BGM Fallback |
| Imagen 3 이미지 | 안전 필터링 적용 | 실루엣/수채화 스타일로 우회 |
...
| Lyria API 문서 | https://ai.google.dev/gemini-api/docs/lyria |
