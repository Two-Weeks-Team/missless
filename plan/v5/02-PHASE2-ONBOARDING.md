...
// privacyStatus를 포함하여 공개/일부공개/비공개 상태를 분류한다.
func (c *YouTubeClient) ListUserVideos(ctx context.Context, token *oauth2.Token) ([]YouTubeVideo, error) {
    ...
    for _, v := range vResp.Items {
        ...
        videos = append(videos, YouTubeVideo{
            ...
            Analyzable:    privacy == "public" || privacy == "unlisted",
        })
    }
...
- [ ] 일부공개(Unlisted) 영상 → 분석 성공
- [ ] 비공개 영상 → 에러 감지 → 갤러리 Fallback 안내
...
기존 `Analyzer→Builder→Matcher` 3단계에서 `VideoAnalyzer→VoiceMatcher` 2단계로 단순화.
- **VideoAnalyzer**: Gemini 3.1 Pro가 YouTube 영상(URL)을 보고 듣고, 대상 인물의 성별·나이대·말투·억양·감정 톤을 텍스트 메타데이터로 추출
- **VoiceMatcher**: 분석 데이터 기반으로 `gemini-live-2.5-flash-native-audio`의 **30개 HD 프리셋 음성** 중 가장 유사한 목소리를 선택(매핑)

> 📌 현 단계에서는 30개 HD 프리셋 음성 매핑으로 구현. (보이스클로닝은 향후 재검토)
...
type VideoInput struct {
    YouTubeURL string // 공개/일부공개: URL 직접
...
// 30개 HD 프리셋 음성 목록 (gemini-live-2.5-flash-native-audio)
var presetVoices = []PresetVoice{
    // 여성 음성
    {Name: "Aoede", Gender: "female", Tone: "warm_bright", AgeHint: "young"},
    {Name: "Charon", Gender: "female", Tone: "calm_deep", AgeHint: "mature"},
    {Name: "Fenrir", Gender: "female", Tone: "energetic", AgeHint: "young"},
    {Name: "Kore", Gender: "female", Tone: "gentle_soft", AgeHint: "young"},
    {Name: "Leda", Gender: "female", Tone: "clear_bright", AgeHint: "young"},
    {Name: "Orus", Gender: "female", Tone: "warm_resonant", AgeHint: "mature"},
    {Name: "Puck", Gender: "female", Tone: "playful", AgeHint: "young"},
    {Name: "Zephyr", Gender: "female", Tone: "airy_light", AgeHint: "young"},
    // 남성 음성
    {Name: "Achilles", Gender: "male", Tone: "strong_warm", AgeHint: "mature"},
    {Name: "Altair", Gender: "male", Tone: "calm_steady", AgeHint: "mature"},
    {Name: "Castor", Gender: "male", Tone: "warm_friendly", AgeHint: "young"},
    {Name: "Helios", Gender: "male", Tone: "bright_energetic", AgeHint: "young"},
    {Name: "Orion", Gender: "male", Tone: "deep_resonant", AgeHint: "mature"},
    {Name: "Perseus", Gender: "male", Tone: "gentle_calm", AgeHint: "young"},
    {Name: "Titan", Gender: "male", Tone: "authoritative", AgeHint: "mature"},
    // ... 나머지 음성은 실제 30개 목록 확인 후 추가
}
...
- [ ] 비공개 영상 터치 → 팝업 안내 (일부공개 변경 / 갤러리 업로드)
