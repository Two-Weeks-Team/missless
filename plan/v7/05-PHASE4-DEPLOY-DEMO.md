...
### DevPost 영상 규칙 (V7 공식 규칙 반영)

- **4분 미만** (3:50 목표 → 10초 여유, **4분 초과 시 이후 부분 심사 안 함**)
- 실제 동작하는 소프트웨어 시연 필수 (목업/모션 그래픽 불가)
- YouTube 또는 Vimeo에 **공개(Public)** 업로드 (비공개/일부공개 시 심사 불가)
...
- Gemini API (5모델 + YouTube URL 직접 분석 + 프리셋 BGM)
...
- [ ] 프리셋 BGM (Cloud Storage 서빙, 6종 분위기별)
...
- **Gemini's interleaved/mixed output**: Live API voice + Tool-triggered image generation
  + emotionally adaptive BGM create a seamless multimodal storytelling stream
...
- **Preset BGM system** with Cloud Storage serving for emotionally adaptive background music
...
- Gemini API (gemini-2.5-flash-native-audio, gemini-2.5-flash-image,
  imagen-4.0-generate-001, gemini-2.5-pro)
- Lyria RealTime (`models/lyria-realtime-exp`) — 참조용 기재 (Go SDK 미지원, 프리셋 BGM 사용)
