...
| Pro (분석) | `gemini-2.5-pro` | 동일 | 비디오/페르소나 분석 |
| Flash Image | `gemini-2.5-flash-image` | 동일 | 빠른 이미지 생성 |
| Imagen 4 | `imagen-4.0-generate-001` | 동일 | 고품질 이미지 — Imagen 3는 2025-11-10 shut down |
| Lyria | `models/lyria-realtime-exp` | 동일 | BGM 생성 (실험적) — **Go SDK 미지원, 프리셋 BGM 사용** |
...
missless-assets/
  ...
  ├── scenes/           # 장면 이미지 (앨범용)
  ├── bgm/              # 프리셋 BGM 파일 (Cloud Storage 서빙)
  └── albums/           # 앨범 공유 이미지
```
