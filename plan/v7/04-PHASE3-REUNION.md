...
## T15. BGM 시스템 + change_atmosphere (프리셋 BGM)

**일수**: 0.5 | **난이도**: 중 | **의존성**: T03, T04

### BGM 전략: 프리셋 BGM + Cloud Storage 서빙

> ⚠️ **V7 변경**: Lyria RealTime API는 WebSocket 전용 (`client.aio.live.music.connect()`)으로,
> Go SDK에서는 지원되지 않음. 따라서 **프리셋 BGM 전략**을 기본으로 채택.
> Lyria 모델(`models/lyria-realtime-exp`)은 DevPost 기술 스택 목록에만 참조용으로 기재.

6종 분위기별 프리셋 BGM 파일을 Cloud Storage에 사전 업로드하고,
`change_atmosphere` Tool 호출 시 mood에 맞는 BGM URL을 즉시 전송한다.
...
func (th *ToolHandler) handleChangeAtmosphere(ctx context.Context, fc *genai.FunctionCall, ws *websocket.Conn) *genai.FunctionResponse {
    ...
    // 프리셋 BGM 매핑 (Cloud Storage URL)
    bgmURL := th.getPresetBGMURL(mood, intensity)

    writeJSON(ws, map[string]any{
        "type": "bgm_change", "mood": mood, "audioUrl": bgmURL,
    })
...
// getPresetBGMURL은 mood/intensity 조합에 맞는 Cloud Storage BGM URL을 반환한다.
func (th *ToolHandler) getPresetBGMURL(mood, intensity string) string {
    // Cloud Storage 프리셋 BGM 경로
    key := fmt.Sprintf("%s_%s", mood, intensity)
    if url, ok := presetBGMMap[key]; ok {
        return url
    }
    // intensity 없는 기본 매핑
    if url, ok := presetBGMMap[mood]; ok {
        return url
    }
    // 최종 Fallback
    return presetBGMMap["nostalgic"]
}

// presetBGMMap은 사전 업로드된 BGM 파일 매핑이다.
// PRE 단계에서 Cloud Storage에 bgm/ 디렉터리로 업로드 완료 필수.
var presetBGMMap = map[string]string{
    "warm":      "https://storage.googleapis.com/missless-assets/bgm/warm-acoustic.mp3",
    "romantic":  "https://storage.googleapis.com/missless-assets/bgm/romantic-piano.mp3",
    "nostalgic": "https://storage.googleapis.com/missless-assets/bgm/nostalgic-strings.mp3",
    "playful":   "https://storage.googleapis.com/missless-assets/bgm/playful-ukulele.mp3",
    "emotional": "https://storage.googleapis.com/missless-assets/bgm/emotional-piano.mp3",
    "farewell":  "https://storage.googleapis.com/missless-assets/bgm/farewell-ambient.mp3",
}
```

### 프론트 BGM 매핑 (로컬 Fallback)

```typescript
// Cloud Storage URL 실패 시 로컬 Fallback
const BGM_PRESET: Record<string, string> = {
...
function handleBGMChange(msg: { mood: string; audioUrl?: string }) {
  const url = msg.audioUrl || BGM_PRESET[msg.mood] || BGM_PRESET.nostalgic;
  bgmPlayer.crossfadeTo(url, { volume: -20 }); // AI 음성보다 -20dB
}
```

### BGM 파일 사전 준비 (PRE 단계 추가)

```bash
# Cloud Storage에 프리셋 BGM 업로드
gsutil cp bgm/*.mp3 gs://missless-assets/bgm/
gsutil iam ch allUsers:objectViewer gs://missless-assets/bgm/
```

> **BGM 소스**: 로열티 프리 BGM (Pixabay, Uppbeat 등) 또는 자체 제작.
> 라이선스 명시 필수 (DevPost 서드파티 통합 선언에 포함).

### 체크포인트

- [ ] AI가 change_atmosphere 호출 → **프리셋 BGM URL 전송 → 브라우저 재생**
- [ ] Cloud Storage BGM 파일 6종 사전 업로드 완료
- [ ] 6종 mood 매핑 동작 확인 (warm/romantic/nostalgic/playful/emotional/farewell)
- [ ] BGM 크로스페이드 전환 동작
...
func (s *Store) Search(ctx context.Context, personaID, topic string) ([]Memory, error) {
    ...
    for {
        ...
        var m Memory
        doc.DataTo(&m)
        // 간단한 키워드 매칭 (벡터 검색은 07-FUTURE-DEV.md 참조)
